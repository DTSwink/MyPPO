...
from kinematic2d.loss_terms import loss_term_keys, normalize_loss_terms_enabled
...
def compute_loss_terms(
    ...
    return {
        ...
        "foot_pin": env.pinned_foot_velocity_loss(action),
...
def _train_loop_impl(
    ...
    while not stop_event.is_set():
        ...
        term_tensors = compute_loss_terms(env, action, limbs)

        loss = torch.zeros((), device=device)
        for key in keys:
            if enabled.get(key, True):
                loss = loss + term_tensors[key]
        ...
        for key in keys:
            interval_terms[key] += float(term_tensors[key].item())
        interval_steps += 1
        ...
        if now - last_checkpoint >= checkpoint_interval:
            ...
            mean_terms = {key: interval_terms[key] / n for key in keys}
            save_checkpoint(checkpoint_dir, **policy.numpy_weights(), step=step, loss=mean_loss)
            append_loss_point(checkpoint_dir, step=step, loss=mean_loss, terms=mean_terms)
            interval_loss = 0.0
            interval_terms = {key: 0.0 for key in keys}
            interval_steps = 0
            last_checkpoint = now
